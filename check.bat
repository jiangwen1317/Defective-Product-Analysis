@echo off
rem 提交前检查入口：hooksPath 自检 + ruff 静态检查 + 存量基线豁免只减不增断言 + tools 门禁脚本回归 + 三个子项目 pytest 套件
...
echo [1/6] ruff 静态检查（AGENTS.md 机械化规则）...
"%PY%" -m ruff check . || goto :fail

echo [2/6] 存量基线豁免只减不增断言 ...
"%PY%" tools\check_exemption_baseline.py || goto :fail

echo [3/6] pytest: tools（门禁脚本回归）...
"%PY%" -m pytest tools -q || goto :fail

echo [4/6] pytest: upper computer ...
"%PY%" -m pytest "upper computer" -q || goto :fail

echo [5/6] pytest: database-analysis ...
"%PY%" -m pytest "database-analysis\tests" -q || goto :fail

echo [6/6] pytest: Log-Download ...
